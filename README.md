...
**重大更新：Debian 13 兼容性和 autobrr 功能**
...
支持 `Ubuntu 16.04 / 18.04`、`Debian 9 / 10 / 11 / 12 / 13` ，`Ubuntu 14.04` 和 `Debian 7/8` 可以用脚本升级，其他系统不支持  
...
注意，`Ubuntu 18.04` 和 `Debian 9/10/11/12/13` 因为 OpenSSL 的原因，只能使用 0.9.6 及以上的版本，更低版本无法直接安装  
...
12. ***autobrr*** 🆕
**`--autobrr`**、**`--no-autobrr`**  
**全新功能！autobrr 是现代化的下载自动化工具，支持 IRC announce 监控和 RSS 订阅**  
主要特性：
- 支持 75+ 私有 PT 站点的 IRC announce 监控
- 实时抓取种子，参与初始 swarm，提高分享率
- 支持 Torznab、Newznab 和标准 RSS 订阅
- 集成多个下载客户端：qBittorrent、Deluge、rTorrent、Transmission、Radarr/Sonarr 等
- 强大的正则表达式过滤功能
- Web UI 管理界面（默认端口 7474）
- 支持通知：Discord、Telegram、Notifiarr、Pushover、Gotify
- 完全使用 Python 3，兼容 Debian 11/12/13

安装后访问：http://your-server-ip:7474  
配置文件位置：`~/.config/autobrr/config.toml`  
官方文档：https://autobrr.com


13. ***FileBrowser Enhanced***  
...
**此外目前这个脚本还有一个作用：当你的 de 失联进不去的时候，用这个脚本限速到比较低的速度就容易进入**  
